...
class LQRSolver_MPS:
    ...
    def value_function(self, t_batch, x_batch, sol_method = 'interpolation'):
        ...
        if sol_method == 'interpolation':
            ...
            time_grid = torch.stack([torch.linspace(0, self.T, N_step, dtype=torch.float32, device = 'mps') for i in [0]])
        ...
        if sol_method == 'direct_calcul':
            ...
            time_grids = torch.stack([torch.linspace(float(t), self.T, 5000, dtype=torch.float32, device = 'mps') for t in t_batch])
